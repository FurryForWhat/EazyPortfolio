...
List all repos owned by `github_username` (use the GitHub MCP tool if connected; otherwise `gh repo list <username> --limit 100 --json name,url`). Drop any repo whose name appears in `exclude_repos`, and drop the portfolio repo itself if it shows up in this list (a repo shouldn't analyze itself).
